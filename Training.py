...
class TextDataset(Dataset):
    ...
    def __init__(
        self, data_path: str, config: RWKV7PhantomConfig, seq_length: int = 512
    ):
        self.config = config
...
class RWKV7PhantomLightning(pl.LightningModule):
    ...
    def compute_ttt_meta_loss(
        self,
        logits: torch.Tensor,
        next_logits: torch.Tensor,
        state_before: List[torch.Tensor],
        state_after: List[torch.Tensor],
    ) -> torch.Tensor:
        """Compute meta-learning loss for TTT.

        Encourages gradient steps to improve next-token prediction. Similar to
        MAML but applied to hidden states.
        """
        ...
        with torch.no_grad():
            improvement = (
                F.softmax(next_logits, dim=-1).max(dim=-1)[0]
                - F.softmax(logits, dim=-1).max(dim=-1)[0]
            )
            meta_loss -= improvement.mean() * 0.1  # Reward improvement

        return (
            meta_loss / count if count > 0 else torch.tensor(0.0, device=logits.device)
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        if (
            self.config.ttt_enabled and np.random.random() < 0.1
        ):  # Meta-learn on 10% of batches
            ...
            loss1 = F.cross_entropy(
                logits1.reshape(-1, logits1.size(-1)), y1.reshape(-1)
            )
            ...
            loss2 = F.cross_entropy(
                logits2.reshape(-1, logits2.size(-1)), y2.reshape(-1)
            )

            # Meta loss
            meta_loss = self.compute_ttt_meta_loss(
                logits1, logits2, state_mid_copy, state_final
            )
    ...
    def configure_optimizers(self):
        ...
        scheduler = {
            "scheduler": CosineAnnealingLR(
                optimizer, T_max=100000, eta_min=self.learning_rate * 0.1
            ),
            "interval": "step",
            "frequency": 1,
        }
...
def train_model(
    ...
    if config.use_svd_compression or config.use_sparsity:
        callbacks.append(
            CompressionCallback(config, compression_epoch=int(num_epochs * 0.8))
        )
...
def load_pretrained_rwkv7(
    ...
    for k, v in state_dict.items():
        ...
        if k.startswith("blocks."):
            ...
            if (
                "att.receptance.weight" in k
                or "att.key.weight" in k
                or "att.value.weight" in k
                or "att.output.weight" in k
            ):
